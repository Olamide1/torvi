...
export default async function TemplatePage({ params }: Props) {
  const { slug } = await params;
  const page = getPageBySlug(slug, "template_page") ?? getPageBySlug(slug, "starter_pack") ?? getPageBySlug(slug);
  if (!page) notFound();
      ...
      <main className="bg-[#F7F6F3] min-h-screen">
        ...
        <div className="max-w-3xl mx-auto px-6 sm:px-10 py-12">
          <div className="space-y-10">
            ...
            {/* Lead capture — bottom (banner variant) */}
            <LeadCapture
              resourceName={page.h1}
              ctaNote={body.ctaNote}
              variant="banner"
            ...
            <div className="border-t border-[#E7E5E4] pt-8">
              ...
              <p className="text-sm text-[#78716C] leading-[1.75] mb-4">
                Templates get you started. Torvi helps you ship a tool that runs the workflow automatically. Four weeks, open enrolment, €200 founding rate.
              </p>
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                >
                  See which track fits your role
                </Link>
                <Link
                  href="/#pricing"
                  className="inline-flex items-center justify-center h-10 px-5 text-sm font-medium rounded border border-[#E7E5E4] text-[#1C1917] hover:border-[#D6D3D1] bg-white transition-colors"
                >
                  View pricing
                </Link>
              </div>
