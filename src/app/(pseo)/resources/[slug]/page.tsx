...
export async function generateStaticParams() {
  return allPSEOPages
    .filter((p) => ["guide_page", "resource_page"].includes(p.pageType))
    .map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const page = getPageBySlug(slug, "guide_page") ?? getPageBySlug(slug, "resource_page");
  if (!page) return {};
  return {
    title: page.title,
      ...
      <main className="bg-[#F7F6F3] min-h-screen">
        ...
        <div className="border-b border-[#E7E5E4] bg-white">
          <div className="max-w-3xl mx-auto px-6 sm:px-10 h-9 flex items-center gap-2 text-xs text-[#78716C]">
            ...
            <Link href="/resources" className="hover:text-[#1C1917] transition-colors">Resources</Link>
            <span className="text-[#D6D3D1]">/</span>
